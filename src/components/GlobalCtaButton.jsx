...
export default function GlobalCtaButton() {
  ...
  const className = isFinalPage
    ? (footer.buttonClassName ??
      'inline-flex h-12 shrink-0 items-center justify-center whitespace-nowrap rounded-full border border-white/20 bg-white px-4 font-semibold uppercase tracking-[0.12em] text-black transition hover:bg-zinc-100 active:scale-[0.98] sm:px-8')
    : [
        'inline-flex h-12 max-w-[calc(100vw-1.5rem)] shrink-0 items-center justify-center whitespace-nowrap rounded-full border px-4 font-semibold uppercase tracking-[0.12em] transition active:scale-[0.98] sm:px-6 sm:tracking-[0.16em]',
        'border-white/25 bg-black/45 text-white backdrop-blur-md hover:bg-black/70 text-xs sm:text-sm',
      ].join(' ')
